...
<script
	src="http://api.map.baidu.com/api?v=2.0&ak=0N0o1Yc0bkGleuYe1ItXFx6V"></script>

</head>

<body onload="simulator()">
	...
	<div id="wrapper">
		...
		<div id="masthead">
			...
			<div class="content_pad">
				...
				<div id="bread_crumbs">
					<a>模拟调度</a> / <a class="current_page">实时地图</a>
				</div>
				<!-- #bread_crumbs -->
		...
		<div id="content" class="xgrid"></div>
		<!-- #content -->
	...
	<div id="map_canvas123456" style="width: 60%; height: 100%"></div>
	...
	<script>
		...
		var lushu4;
		//var points;
		var busName = [ 8, 12, 15, 32, 47, 221, 7, 201, 222, 252, 29, 37, 43,
				45, 4, 12, 16, 26, 6, 11, 36 ];
		var busLineList;
		var centerPoint;
		var map = new BMap.Map('map_canvas123456');
		//var busline;
		var stationsNum;
		var flag = 0;
		var t = 1;
		...
		map.centerAndZoom(new BMap.Point(108.953421, 34.276411), 14);
		busLineList = new BMap.BusLineSearch(map);
		function simulator() {
			busLineList.getBusList(busName[10]);
			busLineList.setGetBusListCompleteCallback(function(busListResult) {
				if (busListResult) {
					var busListItem = busListResult.getBusListItem(0);
					busLineList.getBusLine(busListItem);//必须在里面才能调第二次回调函数
				}
			});
			busLineList
					.setGetBusLineCompleteCallback(function(busLine) {
						var points;
						if (busLine) {
							points = busLine.getPath();

						}
						map.addOverlay(new BMap.Polyline(points, {
							strokeColor : '#FF3030'
						}));
						centerPoint = new BMap.Point(108.953421, 34.276411);
		...
		(function() {
			...
			(function() {
				...
				baidu.dom.getStyle = function(element, key) {
					...
					if (!value) {
						...
						if (fixer) {
							value = fixer.get ? fixer.get(element) : baidu.dom
									.getStyle(element, fixer);
						}
					}
					...
					.extend(
							LuShu.prototype,
							{
								...
								_addMarker : function(callback) {
									...
									this._opts.icon
											&& marker.setIcon(this._opts.icon);
								...
								_addInfoWin : function() {
									var me = this;
									//if(me._opts.defaultContent!== ""){
									var overlay = new CustomOverlay(me._marker
											.getPosition(),
								...
								_getMercator : function(poi) {
									return this._map.getMapType()
											.getProjection().lngLatToPoint(poi);
								},
								/**
								 * 计算两点间的距离
								...
								_getDistance : function(pxA, pxB) {
									return Math.sqrt(Math.pow(pxA.x - pxB.x, 2)
											+ Math.pow(pxA.y - pxB.y, 2));
								},
								...
								_move : function(initPos, targetPos, effect) {
									...
									currentCount = 0,
									//步长，米/秒
									timer = 10, step = this._opts.speed
											/ (1000 / timer),
									//初始坐标
									init_pos = this._projection
											.lngLatToPoint(initPos),
									//获取结束点的(x,y)坐标
									target_pos = this._projection
											.lngLatToPoint(targetPos),
									//总的步长
									count = Math.round(me._getDistance(
											init_pos, target_pos)
											/ step);
									...
									me._intervalFlag = setInterval(
											function() {
												...
												if (currentCount >= count) {
													...
													currentCount++;
													var x = effect(init_pos.x,
															target_pos.x,
															currentCount, count), y = effect(
															init_pos.y,
															target_pos.y,
															currentCount, count), pos = me._projection
								 ...
								 *在每个点的真实步骤中设置小车转动的角度
								 */
								setRotation : function(prePos, curPos,
										targetPos) {
									var me = this;
									var deg = 0;
									//start!
								...
								linePixellength : function(from, to) {
									return Math.sqrt(Math.abs(from.x - to.x)
								...
								_setInfoWin : function(pos) {
									...
									me._overlay.setPosition(pos, me._marker
											.getIcon().size);
									var index = me._troughPointIndex(pos);
									if (index != -1) {
										clearInterval(me._intervalFlag);
										me._overlay
												.setHtml(me._opts.landmarkPois[index].html);
										me._overlay.setPosition(pos, me._marker
												.getIcon().size);
										me._pauseForView(index);
									} else {
										me._overlay
												.setHtml(me._opts.defaultContent);
									}
								},
								/**
								 * 在某个点暂停的时间
								...
								_tween : {
									...
									linear : function(initPos, targetPos,
											currentCount, count) {
										var b = initPos, c = targetPos
												- initPos, t = currentCount, d = count;
										return c * t / d + b;
									}
								},
			...
			function convert(pos) {
				...
				pointss = pos;
				var point = busline.getBusStation(flag).position;
				var num = map.getDistance(point, pointss);
				if (num < 5) {
					sleep(10000);
					flag++;
				}
					...
					.extend(
							CustomOverlay.prototype,
							{
								...
								setPosition : function(poi, markerSize) {
									...
									var px = this._map.pointToOverlayPixel(poi), styleW = baidu.dom
											.getStyle(this._div, 'width'), styleH = baidu.dom
											.getStyle(this._div, 'height');
									overlayW = parseInt(this._div.clientWidth
											|| styleW, 10),
											overlayH = parseInt(
													this._div.clientHeight
															|| styleH, 10);
									this._div.style.left = px.x - overlayW / 2
											+ 'px';
									this._div.style.bottom = -(px.y - markerSize.height)
											+ 'px';
								},
								//设置overlay的内容
